...
def main():
    ...
    # 初始化LLM
    model_path = "/root/models/Qwen3-32B"
    print(f"📚 加载模型: {model_path}")

    llm = LLM(model_path, tensor_parallel_size=8, enforce_eager=False)
    ...
    # 示例1: 数学计算
    messages = [{"role": "user", "content": "请帮我计算 15 * 8 + 32 的结果"}]

    print("👤 用户:", messages[0]["content"])

    response = chat_completion.create(
        messages=messages, tools=tools, temperature=0.0, max_tokens=2048
    )
    ...
    conversation = [
        {
            "role": "user",
            "content": "请查看北京的天气，然后帮我计算一下如果温度是25度，转换为华氏度是多少",
        }
    ]
    ...
    response = chat_completion.create(
        messages=conversation, tools=tools, temperature=0.7, max_tokens=200
    )
    ...
    if assistant_msg.get("tool_calls"):
        ...
        final_response = chat_completion.create(
            messages=conversation, temperature=0.7, max_tokens=100
        )
    ...
    messages = [{"role": "user", "content": "请计算 100 / 4，然后告诉我结果"}]
    ...
    stream = chat_completion.create(
        messages=messages, tools=tools, temperature=0.3, stream=True
    )
