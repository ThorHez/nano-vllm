...
class ToolCallParser:
    ...
    def __init__(self):
        ...
        # 函数调用格式的正则表达式
        self.function_pattern = re.compile(r"(\w+)\s*\(([^)]*)\)", re.DOTALL)
        ...
        # 简单XML格式的正则表达式（tool_call标签包围JSON）
        self.simple_xml_pattern = re.compile(
    ...
    def parse(self, text: str, available_tools: List[str] = None) -> List[ToolCall]:
        ...
        # 尝试简单XML格式解析（tool_call标签包围JSON）
        simple_xml_calls = self._parse_simple_xml_format(text, available_tools)
        tool_calls.extend(simple_xml_calls)
    ...
    def _parse_function_format(
        ...
        for func_name, args_str in matches:
            ...
            tool_call = ToolCall(
                id=str(uuid.uuid4()), name=func_name, arguments=arguments
            )
            tool_calls.append(tool_call)
    ...
    def _parse_xml_format(
        ...
        for name, args_content in matches:
            ...
            tool_call = ToolCall(id=str(uuid.uuid4()), name=name, arguments=arguments)
            tool_calls.append(tool_call)
    ...
    def _parse_simple_xml_format(
        ...
        """解析简单XML格式的工具调用：<tool_call>JSON</tool_call>"""
        tool_calls = []

        matches = self.simple_xml_pattern.findall(text)
    ...
    def contains_tool_call(self, text: str) -> bool:
        ...
        # 检查是否匹配任何格式的模式
        if (
            self.json_pattern.search(text)
            or self.xml_pattern.search(text)
            or self.simple_xml_pattern.search(text)
            or self.function_pattern.search(text)
        ):
            return True
